...
\begin{equation}
P(Y \leq t|J=j) = P(jX \leq t|J=j) (\#eq:conditionaldensity)
\end{equation}
...
If $j=-1\hspace{1mm}$ then $\hspace{1mm}P(jX \leq t|J=j) = P(X \geq -t|J=-1)$

Replacing on equation \@ref(eq:conditionaldensity) we get
...
By replacing on equation \@ref(eq:jminus1) we obtain
...
\begin{align}
f_{Y|J}(t|-1) &= -\frac{d}{dt} \Big(\int_{-\infty}^{-t}f_X(x)dx\Big) \\
\end{align}

\begin{align}
 \hspace{16mm}&= -f_X(-t)\cdot \frac{d}{dt}(-t) \\
 &= f_X(-t) \\
 &= f_X(t)
\end{align}

Regarding the last step in our derivation, it is legitimate due to the fact that the normal distribution is symmetrical. 

&nbsp;

By an analogous approach it is not hard to show that $f_{Y|J}(t|1) = f_X(t)$.
...
This result leaves no doubt: the reason why samples of $X$ have histograms similar to those of the samples of $Y$ is because **$X$ and $Y$ have the same distribution** ---i.e. their distributions are _identical_---.
...
Actually, this question stems from another ---more general--- question: does $0$ correlation imply independence? **Unless some other conditions are met** it does not.
...
We have selected all observations of the sample of $Y$ that, **before being multiplied by $J$**, used to be close to $-1.1$ (between $-1.6$ and $-0.6$). Then, out of those observations, we calculated the fraction of negative values **after being multiplied by $J$.** It is around half, as expected.
...
plot(x_axis, y_axis, xlab = "", ylab= "", main = "Big sample of a bivariate normal vector with 0 correlation")
```
...
**For two random variables to be jointly continuous is necessary** not only that each variable is continuous ---i.e. not discrete---, but also **that the joint range of the variables is a surface** ---i.e. that it is a set with length and breadth---**.** 
...
The silhouette of this sample insinuates the shape of an ellipse^[Remember what we said at the beginning of the article: the density of the bivariate normal distribution is a three-dimensional bell. The contour plots of a bell are ellipses. That is why the joint range is indeed an ellipse.]. **Note that the ellipse does encompass a surface.** In the case of these variables, **the probability is not concentrated on one-dimensional sets but, rather, it is spread across a surface.** Then, these variables do have a joint density, which is that of the bivariate normal distribution.  
...
We have seen that the joint range of two **uncorrelated and independent** standard normal variables has the shape of an ellipse and, therefore, ---when considered jointly--- these variables **follow the bivariate normal distribution.**

In contrast, two **uncorrelated and yet dependent** standard normal variables **do not** ---when considered jointly--- **follow the bivariate normal distribution** ---and actually do not even have a joint density because their joint range is not a surface---.
...
**Uncorrelatedness between two normal variables implies independence if the variables follow the bivariate normal distribution.**

There is something remarkable about the fact that on this last claim we dropped any assumption about the variables involved being standard. Well, that omission didn't happen out of carelessness, it is because **this conclusion applies to any pair of random variables regardless of the parameters of their marginal distributions.** 
